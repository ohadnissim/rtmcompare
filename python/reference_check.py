...
def _estimate_genre(y: np.ndarray, sr: int, bpm: float, deviations: list, lufs: float, dynamic_range: float, correlation: float) -> dict:
    """
    Rule-based genre estimation from acoustic features.

    Features used:
     - BPM (robust estimate, already computed)
     - Sub/bass ratio (kick + sub vs total energy)
     - Spectral centroid (brightness)
     - Spectral flatness (tonal vs noisy)
     - Dynamic range (LRA)
     - Onset density (notes per second)
     - Vocal-band energy (200-4k Hz)

    Returns: {primary: str, confidence: float, candidates: [(name, score)]}

    Conservative — when features conflict, returns "Unclear" rather than
    guessing.
    """
    try:
        # Use a 60s representative window from the middle to avoid intros
        total = len(y)
        if total > sr * 90:
            start = total // 3
            y_use = y[start:start + sr * 60]
        else:
            y_use = y

        # Spectral features (mean over frames)
        S = np.abs(librosa.stft(y_use, n_fft=2048, hop_length=512)) + 1e-10
        ...
        sub_e     = band_energy(20, 80)
        bass_e    = band_energy(80, 250)
        low_mid_e = band_energy(250, 800)
        mid_e     = band_energy(800, 3000)
        high_e    = band_energy(3000, 8000)
        air_e     = band_energy(8000, 16000)
        total_e   = sub_e + bass_e + low_mid_e + mid_e + high_e + air_e + 1e-10

        sub_bass_ratio = (sub_e + bass_e) / total_e
        mid_ratio      = mid_e / total_e
        air_ratio      = air_e / total_e

        # Spectral centroid (brightness, Hz)
        centroid = float(np.mean(librosa.feature.spectral_centroid(y=y_use, sr=sr)))
        # Spectral flatness (0=tonal, 1=noisy)
        flatness = float(np.mean(librosa.feature.spectral_flatness(y=y_use)))
        # Onset density (per second) — rhythmic complexity
        try:
            onsets = librosa.onset.onset_detect(y=y_use, sr=sr, units='time')
            onset_rate = len(onsets) / (len(y_use) / sr) if len(y_use) > 0 else 0.0
        except Exception:
            onset_rate = 0.0

        # Harmonic / percussive separation strength — electronic music is
        # typically harmonic-heavy with strong percussion too.
        try:
            H, P = librosa.effects.hpss(y_use, margin=3.0)
            perc_energy = float(np.sqrt(np.mean(P ** 2)))
            harm_energy = float(np.sqrt(np.mean(H ** 2)))
            hp_ratio = harm_energy / (perc_energy + 1e-8)
        except Exception:
            hp_ratio = 1.0

        # Tonal mid region (vocals live here)
        vocal_band = mid_ratio

        # Build scores for each genre (0-100).
        scores = {}

        # Electronic / EDM: 118-135 BPM, heavy bass+sub, compressed (low LRA),
        # bright / high air energy, low harmonic ratio (lots of percussion).
        s = 0
        if 118 <= bpm <= 140: s += 35
        elif 108 <= bpm < 118 or 140 < bpm <= 150: s += 15
        if sub_bass_ratio > 0.30: s += 20
        if dynamic_range < 6: s += 15
        if air_ratio > 0.05: s += 10
        if centroid > 2200: s += 10
        if onset_rate > 4: s += 10
        scores["Electronic / EDM"] = s

        # Hip-hop / Trap: 70-100 BPM (or 140-160 for trap at half-time),
        # very heavy sub, compressed, vocal-forward mids.
        s = 0
        if 70 <= bpm <= 100: s += 30
        elif 140 <= bpm <= 170: s += 25  # trap at half-time
        if sub_bass_ratio > 0.35: s += 25
        if dynamic_range < 7: s += 15
        if vocal_band > 0.20: s += 15
        if onset_rate < 6: s += 10
        scores["Hip-Hop / Trap"] = s

        # Pop: 90-130 BPM, balanced spectrum, moderate compression,
        # strong vocal band, good air.
        s = 0
        if 90 <= bpm <= 130: s += 25
        if 0.20 < sub_bass_ratio < 0.38: s += 15
        if vocal_band > 0.18: s += 20
        if 4 < dynamic_range < 9: s += 15
        if air_ratio > 0.03: s += 10
        if 1400 < centroid < 2800: s += 15
        scores["Pop"] = s

        # Rock / Indie: 100-170 BPM, guitar-heavy mids, moderate-to-high LRA,
        # less sub than EDM/hip-hop, strong transients.
        s = 0
        if 100 <= bpm <= 170: s += 20
        if 0.15 < sub_bass_ratio < 0.32: s += 15
        if mid_ratio > 0.25: s += 20
        if dynamic_range > 6: s += 15
        if onset_rate > 3: s += 10
        if flatness < 0.25: s += 10
        if hp_ratio < 1.5: s += 10
        scores["Rock / Indie"] = s

        # R&B / Soul: 60-100 BPM, warm mids and bass, vocal-centric,
        # moderate dynamics.
        s = 0
        if 60 <= bpm <= 100: s += 25
        if vocal_band > 0.22: s += 25
        if 0.22 < sub_bass_ratio < 0.38: s += 15
        if 5 < dynamic_range < 10: s += 15
        if flatness < 0.2: s += 10
        if centroid < 2200: s += 10
        scores["R&B / Soul"] = s

        # Ambient / Downtempo / Cinematic: slow or no clear tempo, high LRA,
        # sustained pads, low onset density.
        s = 0
        if bpm < 75 or bpm == 0: s += 25
        if dynamic_range > 9: s += 25
        if onset_rate < 2.5: s += 25
        if flatness < 0.15: s += 15
        if mid_ratio > 0.25: s += 10
        scores["Ambient / Cinematic"] = s

        # Acoustic / Folk / Singer-Songwriter: variable tempo, high LRA,
        # tonal (low flatness), strong mids, low sub.
        s = 0
        if 70 <= bpm <= 140: s += 15
        if dynamic_range > 8: s += 25
        if sub_bass_ratio < 0.22: s += 20
        if flatness < 0.15: s += 15
        if vocal_band > 0.25: s += 15
        if correlation > 0.6: s += 10
        scores["Acoustic / Folk"] = s

        # Jazz: moderate tempo, very high LRA, low sub, tonal, complex onsets.
        s = 0
        if 80 <= bpm <= 180: s += 15
        if dynamic_range > 10: s += 25
        if sub_bass_ratio < 0.25: s += 20
        if flatness < 0.15: s += 15
        if onset_rate > 4: s += 10
        if vocal_band > 0.20: s += 5
        scores["Jazz"] = s

        # Classical / Orchestral: any tempo, very high LRA, low sub,
        # very tonal (low flatness), wide stereo.
        s = 0
        if dynamic_range > 12: s += 35
        if sub_bass_ratio < 0.20: s += 20
        if flatness < 0.1: s += 20
        if correlation < 0.5: s += 15  # wide stereo
        if vocal_band < 0.20: s += 10
        scores["Classical / Orchestral"] = s
        ...
        # Require a clear winner — margin of ≥ 15 points
        if top_score < 40 or (top_score - second_score) < 10:
            primary = "Unclear"
            confidence = max(0.0, min(1.0, top_score / 100.0))
        else:
            primary = top_name
            # Confidence scales with top_score and margin
            margin = top_score - second_score
            confidence = max(0.2, min(1.0, (top_score + margin) / 150.0))

        return {
            "primary": primary,
            "confidence": round(confidence, 2),
            "candidates": [{"name": n, "score": int(s)} for n, s in ranked[:4]],
            "features": {
        ...
        return {"primary": "Unclear", "confidence": 0.0, "candidates": [], "features": {}, "error": str(e)}
