...
function buildShareHtml(title: string, reportJson: string): string {
  ...
  // LOW-2: explicit escapeHtml on all template-interpolated strings, even
  // numeric-looking ones, so this is safe if types ever loosen in future.
  const visqolLine = overall.visqol_mos != null
    ? `<p style="margin:8px 0;font-size:12px;color:rgba(208,176,102,0.7)">ViSQOL perceptual match: <strong>${escapeHtml(Number(overall.visqol_mos).toFixed(2))}/5.0</strong></p>`
    : ''
  const dateFmt = escapeHtml(new Date().toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' }))
...
${matchScore != null ? `<div class="score">${escapeHtml(String(Math.round(Number(matchScore))))}%</div><div class="score-label">Match Score</div>` : ''}
${visqolLine}
<table>
