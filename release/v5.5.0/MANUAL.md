...
**v5.5.0** · macOS arm64 + Windows x64 · everything runs locally

---

## What's new in 5.5.0

*The headline stuff, in one place.*

- **Design bump** — Reference history is a dropdown now. "+ New
  analysis" and a Search palette (⌘K) live in the header. Press `?`
  for the keyboard-shortcut sheet. Advanced QC remembers its state
  across sessions. EQ Preview's "Level matched" toggle is a pill,
  not a hidden gear-icon setting.
- **Click & glitch detector** — replaced with FLOW v2 (LPC residual,
  Godsill & Rayner 1998). No more drum false positives.
- **Stem separator** — BS-RoFormer 4-stem (SDR 9.66 on MUSDB18HQ,
  up from ~7.5 with Demucs). Falls back to Demucs if it can't load.
- **AI detection removed** — see the box at the end of the
  Reference Library section.
- **Quieter scans** — Ceiling and ADM warnings no longer fire on
  every non-Atmos file.
- **RTMprofile (1.4.0)** — production model-cache discovery fixed
  so standalone `RTMprofile.app` finds its weights on first launch.
...
MUSDB18HQ — clean enough that the per-stem masking, EQ Preview
level-match, and Match-tab comparisons actually trust the
isolation. Falls back to Demucs if BS-RoFormer can't load.

> **Heads-up — AI detection has been removed in 5.5.0.** The
> 24-detector ensemble we'd been working on would have added
> 1.1 GB to the bundle for one optional feature most engineers
> don't act on. We pulled it. Sit on 5.4.0 if you relied on it.
