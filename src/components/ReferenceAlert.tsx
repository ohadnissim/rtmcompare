...
export default function ReferenceAlert({ check, labelA }: Props) {
 ...
 <span className="text-xs ml-1.5" style={{ color: 'var(--color-sand-300)' }}>{check.summary}</span>
 </div>
 <svg
 ...
 {check.stats.clip_count > 0 && <span style={{ color: 'var(--color-danger)' }}>Clips: {check.stats.clip_count}</span>}
 </div>
 ...
 <p className="text-xs" style={{ color: w.severity === 'warning' ? 'var(--color-sand-100)' : 'var(--color-sand-300)' }}>
 {w.message}
 </p>
 <p className="text-[11px] font-display italic" style={{ color: '#6b645d' }}>
 {w.suggestion}
 </p>
 </div>
 ))}
