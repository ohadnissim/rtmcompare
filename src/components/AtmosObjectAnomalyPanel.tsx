...
export default function AtmosObjectAnomalyPanel({ result }: Props) {
 ...
 <span className="text-[10px] uppercase tracking-[0.14em]" style={{ color: 'var(--color-text-muted)' }}>Per-object anomalies</span>
 {blockCount > 0 && (
 <span className="text-[10px] px-2 py-0.5 rounded" style={{ color: 'var(--color-danger)', backgroundColor: 'rgba(224,90,90,0.1)' }}>
 {blockCount} block{blockCount === 1 ? '' : 's'}
 </span>
 )}
 {warnCount > 0 && (
 <span className="text-[10px] px-2 py-0.5 rounded" style={{ color: 'var(--color-accent)', backgroundColor: 'rgba(197,165,90,0.1)' }}>
...
function AnomalyRow({ a }: { a: Anomaly }) {
 const color = a.severity === 'block' ? 'var(--color-danger)' : 'var(--color-accent)'
 ...
 <span className="font-mono text-[10px]" style={{ color: 'var(--color-text-muted)' }}>{a.channel}</span>
 <span style={{ color: 'var(--color-text-primary)' }}>{a.label || a.role}</span>
 <span className="font-mono text-[9px] uppercase tracking-[0.14em]" style={{ color }}>{a.kind}</span>
 </div>
 <div className="text-[10px] mt-1 ml-6" style={{ color: 'var(--color-text-muted)' }}>{a.detail}</div>
 </div>
 )
}
