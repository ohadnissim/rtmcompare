...
export default function CategoryCard({ category, labelA, labelB }: Props) {
 ...
 <div className="mt-2 pt-2 flex gap-4" style={{ borderTop: '1px solid rgba(168,161,150,0.08)' }}>
  ...
  <div className="flex-1">
   ...
   {isFinite(category.centroid_b) ? (category.centroid_b >= 1000 ? `${(category.centroid_b / 1000).toFixed(1)} kHz` : `${Math.round(category.centroid_b)} Hz`) : '—'}
   </div>
   <div className="text-[9px] font-mono" style={{ color: 'var(--color-text-dim)' }}>
   {(() => {
    const d = category.centroid_b - category.centroid_a
    if (!isFinite(d) || Math.abs(d) < 10) return '0'
  ...
  <div className="flex-1">
   ...
   {(() => {
    ...
    if (!isFinite(d) || Math.abs(d) < 0.05) return '0.0'
  ...
  <div className="flex-1">
   ...
   <div className="text-[9px] font-mono" style={{ color: 'var(--color-text-dim)' }}>
   {isFinite(category.pan_b) ? category.pan_b.toFixed(2) : '—'}
   </div>
  </div>
  )}
 </div>
